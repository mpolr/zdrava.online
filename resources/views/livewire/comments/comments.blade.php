<div>
    <div class="container mx-auto mt-0 items-left">
        ...
        <div class="space-y-4">
            ...
            @foreach ($comments as $comment)
                @if ($onlyLast)
                    @if($loop->last)
                        <div class="flex-col w-full p-4 bg-white border-b-2 border-r-2 border-gray-200 sm:px-4 md:px-2 sm:rounded-lg sm:shadow-sm dark:bg-gray-800 dark:border-gray-800">
                            <div class="flex flex-row ml-2">
                                ...
                                <div class="flex-col mt-1">
                                    <div class="flex items-center flex-1 px-4 font-bold leading-tight">
                                        <a href="{{ route('athlete.profile', $comment['userId']) }}" class="dark:text-gray-400">{{ $comment['author'] }}</a>
                                        <span class="ml-2 text-xs font-normal text-gray-500">{{ $comment['date'] }}</span>
                                    </div>
                                    <div class="flex-1 px-2 ml-2 text-sm font-medium leading-loose text-gray-600 dark:text-gray-300">
                                        {{ $comment['text'] }}
                                    </div>
                                ...
                                <div class="flex flex-row">
                                    ...
                                    <div class="flex-col mt-1">
                                        <div class="flex items-center flex-1 px-2 font-medium leading-tight">
                                            <a href="{{ route('athlete.profile', $reply['userId']) }}" class="dark:text-gray-400">{{ $reply['author'] }}</a>
    ...
    <form wire:submit.prevent="addComment" class="mt-4">
        <label>
            <textarea wire:model="commentContent" class="border border-gray-300 rounded-lg p-2 w-full dark:bg-gray-800 dark:text-gray-200" rows="3" placeholder="Введите комментарий"></textarea>
        </label>
