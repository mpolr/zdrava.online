@foreach($model->likes as $like)
    <img class="h-5 w-5 flex-none rounded-full bg-gray-50" src="{{ $like->user->getPhoto() }}" alt="{{ $like->user->getFullName() }}" >
@endforeach
...
@can('like', $model)
    @if($model->getUser()->id !== auth()->id())
        <form action="{{ route('activities.like') }}" method="POST">
            ...
            <button class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-500 bg-transparent hover:bg-gray-900 hover:text-white focus:z-10 focus:ring-2 focus:ring-gray-500 focus:bg-gray-900 focus:text-white dark:border-white dark:text-white dark:hover:text-white dark:hover:bg-gray-700 dark:focus:bg-gray-700">
